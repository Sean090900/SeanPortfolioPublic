...
class InferenceService:
    ...
    def run(self) -> None:
        ...
        **Example Class Usage:**
        ...
        # Retieve frame
        frame_count = 0
        for frame in stream.capture_video():

            # Skip frame if None
            if frame is None:
                frame_count += self.drop_rate
                continue
            ...
            bboxes, class_ids, confidence_scores, class_scores = self.detector.post_process(output)

            # Supress boxes using NMS
            filtered_bboxes, filtered_class_ids, filtered_scores, filtered_class_scores = self.nms.filter(bboxes, class_ids, confidence_scores, class_scores)

            # Draws boxes onto frame
            processed_frame = self.draw_boxes(frame, filtered_bboxes, filtered_class_ids, filtered_scores)
...
if __name__ == "__main__":
    ...
    VIDEO_SOURCE = '/Users/seandickson/JohnsHopkinsScripts/CreatingAIEnabledSystems/techtrack-Sean090900/techtrack/storage/test_videos/worker-zone-detection.mp4'
    ...
    service = InferenceService(stream, model, nms, save_dir='storage/detections', drop_rate=60)
    service.run()
