...
def calculate_iou(boxA, boxB):
    ...
    areaA = max(0, Aw) * max(0, Ah)
    areaB = max(0, Bw) * max(0, Bh)
...
def match_detections(boxes, classes, scores, cls_scores, gt_boxes, gt_classes, map_iou_threshold, eval_type="class_scores"):
    ...
    try:
        num_classes = int(np.asarray(cls_scores[0][0]).shape[-1])
    except Exception:
        num_classes = 0
    zero_vec = np.zeros(num_classes, dtype=float) if num_classes else np.array([])

    for i in range(len(gt_boxes)):
        grt_boxes = gt_boxes[i]
        det_boxes = boxes[i]

        # Build all (det, gt, iou) above threshold
        candidates = []
        for g in range(len(grt_boxes)):
            for d in range(len(det_boxes)):
                iou = calculate_iou(grt_boxes[g], det_boxes[d])
                if iou >= map_iou_threshold:
                    candidates.append((d, g, iou))
                    
        # Greedy one-to-one by IoU desc
        candidates.sort(key=lambda x: x[2], reverse=True)
        used_dets = set()
        used_gts = set()
        det_to_gt = {}

        for d, g, _ in candidates:
            if d in used_dets or g in used_gts:
                continue
            det_to_gt[d] = g
            used_dets.add(d)
            used_gts.add(g)

        # Emit rows for each detection
        if eval_type == "class_scores":
            for d in range(len(det_boxes)):
                if d in det_to_gt:
                    g = det_to_gt[d]
                    y_true.append(gt_classes[i][g])
                else:
                    y_true.append(-1)
                pred_scores.append(np.asarray(cls_scores[i][d], dtype=float))
        elif eval_type == "objectness":
            for d in range(len(det_boxes)):
                y_true.append(1 if d in det_to_gt else 0)
                pred_scores.append(np.array([1.0 - scores[i][d], scores[i][d]], dtype=float))
        elif eval_type == "combined":
            for d in range(len(det_boxes)):
                combined = np.asarray(cls_scores[i][d], dtype=float) * float(scores[i][d])
                if d in det_to_gt:
                    g = det_to_gt[d]
                    y_true.append(gt_classes[i][g])
                else:
                    y_true.append(-1)
                pred_scores.append(combined)
        else:
            raise ValueError(f"Unknown eval_type: {eval_type}")

        # Add FNs for any GT that wasn't matched
        if eval_type in ("class_scores", "combined"):
            for g in range(len(grt_boxes)):
                if g not in used_gts:
                    y_true.append(gt_classes[i][g])
                    pred_scores.append(zero_vec.copy())
...
def calculate_precision_recall_curve(y_true, pred_scores, num_classes=20):
    ...
    # Binarize true labels for per-class evaluation
    y_true_bin = label_binarize(y_true, classes=np.arange(num_classes))
    if y_true_bin.shape[1] != num_classes:

        # In case label_binarize drops empty classes, pad back to num_classes
        padded = np.zeros((y_true_bin.shape[0], num_classes), dtype=y_true_bin.dtype)
        if y_true_bin.shape[1] > 0:

            # label_binarize maps classes in ascending order; align columns accordingly
            present_classes = np.unique(y_true)
            padded[:, present_classes] = y_true_bin
        y_true_bin = padded
    ...
    n_samples = pred_scores.shape[0]

    for c in range(num_classes):
        # Scores and binary truths for class c
        scores_c = pred_scores[:, c]
        y_c = y_true_bin[:, c].astype(int)

        # Sort by score descending (stable sort keeps order among ties)
        order = np.argsort(-scores_c, kind="mergesort")
        s_sorted = scores_c[order]
        y_sorted = y_c[order]

        # Cumulative TP / FP as we lower the threshold down the sorted list
        tp_cum = np.cumsum(y_sorted)
        fp_cum = np.cumsum(1 - y_sorted)
        total_pos = int(y_c.sum())

        prec_list = []
        rec_list = []
        thr_list = []

        # For each threshold (each predicted score in descending order)
        for k in range(n_samples):
            ...
            rec_list.append(rec)
            thr_list.append(float(s_sorted[k]))

        # Append an extra threshold at 0 as requested
        thr_list.append(0.0)

        # Precision/recall when threshold == 0 (everything predicted positive)
        TP_all = total_pos
        FP_all = n_samples - total_pos
        prec_all = TP_all / (TP_all + FP_all) if (TP_all + FP_all) > 0 else 0.0
        rec_all = TP_all / (TP_all + 0) if TP_all > 0 else 0.0

        prec_list.append(prec_all)
        rec_list.append(rec_all)

        precision[c] = prec_list
        recall[c] = rec_list
        thresholds[c] = np.asarray(thr_list, dtype=float)
