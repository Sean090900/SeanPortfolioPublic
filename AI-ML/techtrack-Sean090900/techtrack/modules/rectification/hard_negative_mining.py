...
class HardNegativeMiner:
    ...
    NOTE: DO NOT change this implementation! It's already done for you! All you have to do
    is complete the `sample_hard_negatives` method.
    ...
    def sample_hard_negatives(self, num_hard_negatives, criteria):
        ...
        """
        # Build/refresh table from the dataset
        self.__construct_table()

        # Validate criteria
        if criteria not in self.table.columns:
            # Show just the metric columns to help the user pick a valid criteria
            metric_cols = [c for c in self.table.columns if c not in ("annotation_file", "image_file")]
            raise KeyError(
                f"Criteria '{criteria}' not found in table columns."
                f"Available criteria: {metric_cols}"
            )

        # Clean the data for the chosen criteria
        df = self.table.copy()
        df[criteria] = pd.to_numeric(df[criteria], errors="coerce")
        df = df.replace([np.inf, -np.inf], np.nan).dropna(subset=[criteria])

        # Nothing to return if no valid rows
        if df.empty or num_hard_negatives <= 0:
            return df.iloc[0:0].reset_index(drop=True)

        # Sort by criteria (descending = "harder"). Add a deterministic tiebreaker if present.
        sort_cols = [criteria]
        ascending = [False]
        if "image_file" in df.columns:
            sort_cols.append("image_file")
            ascending.append(True)

        df_sorted = df.sort_values(by=sort_cols, ascending=ascending, kind="mergesort")

        # Select the top-k
        k = min(int(num_hard_negatives), len(df_sorted))
        return df_sorted.head(k).reset_index(drop=True)
