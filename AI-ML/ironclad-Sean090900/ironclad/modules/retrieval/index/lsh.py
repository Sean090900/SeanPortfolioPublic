...
class FaissLSH:
    ...
    def add_embeddings(self, embeddings, metadata):
        ...
        elif len(embeddings[0]) != self.dim:
            raise ValueError(f'Embeddings dimmesion = {len(embeddings[0])}, but specified dimmension = {self.dim}. These must be equal')

        # Raise ValueError if the number of embeddings and metadata entries do not match.
        if len(embeddings) != len(metadata):
            raise ValueError(f"Length of embeddings and length of metadata must match: {len(embeddings)} != {len(metadata)}")
