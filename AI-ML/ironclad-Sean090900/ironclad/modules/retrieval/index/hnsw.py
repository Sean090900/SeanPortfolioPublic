...
class FaissHNSW:
    ...
    def __init__(self, dim, **kwargs):
        ...
        """
        # Optional params via **kwargs
        self.m = kwargs.get('M')
        if not self.m:
            self.m = 32
        self.efConstruction = kwargs.get('efConstruction')
        self.metric = kwargs.get('metric')
        if self. metric and self.metric not in ['euclidean', 'cosine']:
            raise ValueError('Euclidean is the only acceptable metric for FaissHNSW.')
    ...
    def add_embeddings(self, new_embeddings, new_metadata):
        ...
        """
        # Raise ValueError if an embedding does not match the specified dimensionality.
        if len(new_embeddings) == 0:
            raise ValueError('Embeddings is empty.')
        elif len(new_embeddings[0]) != self.dim:
            raise ValueError(f'Embeddings dimmesion = {len(new_embeddings[0])}, but specified dimmension = {self.dim}. These must be equal')

        # Raise ValueError if the number of embeddings and metadata entries do not match.
        if len(new_embeddings) != len(new_metadata):
            raise ValueError(f"Length of embeddings and length of metadata must match: {len(new_embeddings)} != {len(new_metadata)}")
    ...
    def get_metadata(self, idx):
        ...
        if idx not in range(len(self.metadata)):
            raise IndexError(f"{idx} is out of range to self.metadata of length: {len(self.metadata)}")
