...
Usage:
    ...
    Sample curl command for /add:
        ...
        curl -X POST -F "image=@storage/multi_image_gallery/Aaron_Sorkin/Aaron_Sorkin_0001.jpg" -F "identity=Aaron_Sorkin" http://localhost:5000/add
    Sample curl command for /identify:
        curl -X POST -F "probe=@/path/to/image.jpg" -F "k=3" http://localhost:5000/identify
        curl -X POST -F "probe=@storage/multi_image_gallery/Aaron_Sorkin/Aaron_Sorkin_0001.jpg" -F "k=3" http://localhost:5000/identify
"""
import os
import numpy as np
...
index = FaissBruteForce(dim=160, metric=SIMILARITY_MEASURE)
...
def identify():
    ...
    try:
        ...
        return jsonify({
            "error": "Failed to return search data.",
            "details": str(e)
        }), 500
