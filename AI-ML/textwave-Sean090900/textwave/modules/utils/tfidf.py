...
class TF_IDF:
    ...
    def _tokenize(self, text: str): 
        # NOTE: Initial params: def _tokenize(self, documents: List[str]): 
        """
    ...
    def fit(self, documents: List[str]):
        # NOTE: Initial params: def fit(self, document: str):
        """
        ...
        idf = [0. for _ in range(len(vocabulary))]
        ...
        for token, index in vocabulary.items():
            ...
            # Add IDF score to dict
            idf[index] = math.log(total_documents / (document_frequency + 1)) + 1

        # Save fitted vocab, fitted idf, return self
        self.vocabulary_ = vocabulary
        self.idf_ = np.array(idf)
        return self
    ...
    def transform(self, document):
        ...
        #     raise AttributeError('You must fit the TF_IDF object before transforming.')
        ...
        for token in set(tokens):
            if token in self.vocabulary_:
                ...
                tf = Counter(tokens)[token] / len(tokens)
                tf_idf = tf * self.idf_[index]
                transformed_document[index] = tf_idf
