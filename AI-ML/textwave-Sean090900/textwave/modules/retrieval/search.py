...
class FaissSearch:
    ...
    def search(self, query_vector, k=5):
        ...
        if self.metric == 'euclidean':
            ...
            # print(indices_candidate)
            # print(query_vector[0])

            # indices = indices_candidate
            # distances = distance_candidate
            # Compute cosine distance between query_vector and neighbors
            # for index in indices:
                

            # Re-organize based on cosine_distance
        ...
        # # Remove -1s from indicies if necessary
        # indices = [idx for idx in indices if idx not -1]

        # Retrieve metadata for the nearest neighbors.
        metadata_results = [self.faiss_index.get_metadata(int(i)) for i in indices[0] if i != -1]
        return distances, indices, metadata_results

    # def _compute_cosine(self, query_vector, nearest_vectors):
    #     """
    #     Compute Cosine distance between the query vector and a set of candidate vectors.
